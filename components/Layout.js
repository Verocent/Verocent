...
export default function Layout({
  ...
  data, currentUser, onLogout,
  allowedModules=[], children
}) {
  ...
  const compDue     = compliance.filter(c=>{
    const d = Math.round((new Date(c.expiry)-new Date())/86400000);
    return d<=90;
  }).length;
    ...
    <div style={{
      ...
      <div style={{padding:'20px 16px 14px',
        ...
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:8}}>
          <div style={{
            width:44, height:44, background:C.gold,
          ...
          <div>
            <div style={{color:'#fff',fontWeight:800,fontSize:13,
              fontFamily:'sans-serif'}}>VEROCENT</div>
            <div style={{color:C.gold,fontWeight:600,fontSize:10,
              fontFamily:'sans-serif'}}>PURE ESSENCE ERP</div>
          </div>
        </div>
        <div style={{color:'rgba(255,255,255,0.35)',fontSize:10,
          fontFamily:'sans-serif'}}>🇳🇬 Kaduna, Nigeria</div>
      </div>
          ...
          return (
            <button key={m.id} onClick={()=>handleNav(m.id)}
              style={{
                ...
                background: active
                  ?'rgba(212,160,23,0.15)':'transparent',
                borderLeft: active
              ...
              {m.id==='sales' && outstanding>0 && (
                <span style={{background:C.red,color:'#fff',
                  borderRadius:10,padding:'1px 7px',
                  fontSize:10,fontWeight:800}}>
                  {outstanding}
                </span>
              )}
              {m.id==='inventory' && lowStock>0 && (
                <span style={{background:C.red,color:'#fff',
                  borderRadius:10,padding:'1px 7px',
                  fontSize:10,fontWeight:800}}>
                  {lowStock}
              ...
              {m.id==='compliance' && compDue>0 && (
                <span style={{background:C.amber,color:'#fff',
                  borderRadius:10,padding:'1px 7px',
                  fontSize:10,fontWeight:800}}>
                  {compDue}
                </span>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{display:'flex',alignItems:'center',
            ...
            <button onClick={()=>setMobileOpen(!mobileOpen)}
              style={{
                ...
                color:C.green, padding:4,
                borderRadius:6, lineHeight:1, flexShrink:0,
              }}>☰</button>
            <div style={{minWidth:0}}>
              <div style={{
                fontWeight:800, fontSize:15, color:C.body,
          ...
          <div style={{display:'flex',gap:8,
            ...
            {lowStock>0&&(
              <div style={{
                background:'#FEF9E7', color:C.amber,
                borderRadius:8, padding:'4px 10px',
                fontSize:11, fontWeight:700,
                fontFamily:'sans-serif', whiteSpace:'nowrap',
            ...
            <div style={{
              background:'#E8F5EE', color:C.dark,
              borderRadius:8, padding:'4px 10px',
